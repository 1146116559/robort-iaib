...
def Merge_Thinning_Edges(img1, img2):
	...
	return edges, bitwiseAnd3

def HoughLinesP(img, Original):
	lines = cv2.HoughLinesP(img, 1.0, np.pi/180, 100, minLineLength=180, maxLineGap=250)
	...
	if lines is not None:       
		for line in lines:
			...
			if (slope == 0):
				...
				if 0 <= abs(angle_in_degrees) <= 40 or 50 <= abs(angle_in_degrees) <= 90:
	...
	if check_x == 1 and check_y == 1:
		for k in range(len(Horizontal)):
			for m in range(len(vertical)):  
				for j in range(m+1,len(vertical)):			
	...
	if __name__ == '__main__':
		...
		##if ret == False:   
			...
			if Lines is not None:
				...
				new_img = Intersection(new_img, data, Horizontal, vertical, check_x, check_y)
